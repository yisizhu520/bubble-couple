/**
 * Enemy AI Core Algorithms
 * 
 * Pure AI logic functions that can be shared between:
 * - Client-side enemy AI (systems/EnemyAI.ts)
 * - Server-side enemy AI (server/src/rooms/BubbleRoom.ts)
 * 
...
/** Direction enum values as strings (compatible with both client and server) */
...
/**
 * Find the nearest alive entity from a list.
...
/**
 * Simple chase direction (for server-side use).
 * Just picks direction toward target without obstacle avoidance.
 */
